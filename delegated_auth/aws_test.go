	...
	testCases := []testCase{
		{
			...
			awsAuth:        AWSAuth{},
			expectedHost:   "sts.amazonaws.com",
			expectedRegion: "us-east-1",
			expectedUrl:    "https://sts.amazonaws.com",
		},
		{
			name:           "Regional AWS Region",
			awsAuth:        AWSAuth{AwsRegion: "eu-west-1"},
			expectedHost:   "sts.eu-west-1.amazonaws.com",
			expectedRegion: "eu-west-1",
			expectedUrl:    "https://sts.eu-west-1.amazonaws.com",
		},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			stsFullURL, region, host := tc.awsAuth.getConnectionParameters()
			if stsFullURL != tc.expectedUrl {
				t.Errorf("expected URL %s, got %s", tc.expectedUrl, stsFullURL)
			}
			if region != tc.expectedRegion {
				t.Errorf("expected region %s, got %s", tc.expectedRegion, region)
			}
...
func TestGetCredentials(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			t.Setenv(awsAccessKeyName, tc.awsAccessKeyName)
			t.Setenv(awsSecretKeyName, tc.awsSecretKeyName)
			t.Setenv(awsSessionToken, tc.awsSessionToken)
	...
	testCases := []testCase{
		{
			...
			awsAuth: AWSAuth{AwsRegion: "us-east-1"},
			creds: &Credentials{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if data != nil {
				...
				if data.Method != http.MethodPost {
					t.Errorf("expected method %s, got %s", http.MethodPost, data.Method)
				}
				...
				if string(urlDecoded) != stsFullUrl {
					t.Errorf("expected URL %s, got %s", stsFullUrl, urlDecoded)
				}
				...
				if headers["host"][0] != host {
					t.Errorf("expected host %s in headers, got %s", host, headers["host"][0])
				}
				...
				if !strings.Contains(authorization, region) {
					t.Errorf("expected region %s in Authorization header, got %s", region, headers["Authorization"][0])
				}
