...
func testMonitor(ctx context.Context, t *testing.T) datadog.Monitor {
	return datadog.Monitor{
		...
		Options: &datadog.MonitorOptions{
			...
			TimeoutH:             *common.NewNullableInt64(common.PtrInt64(1)),
...
func TestMonitorLifecycle(t *testing.T) {
	...
	if err != nil {
		t.Errorf("Error updating Monitor %v: Response %v: %v", monitor.GetId(), err.(common.GenericOpenAPIError).Body(), err)
	}
	assert.Equal(200, httpresp.StatusCode)
	...
	if err != nil {
		t.Errorf("Error fetching Monitor %v: Response %v: %v", monitor.GetId(), err.(common.GenericOpenAPIError).Body(), err)
	}
	assert.Equal(200, httpresp.StatusCode)
	...
	if err != nil {
		t.Errorf("Error fetching monitors: Response %s: %v", err.(common.GenericOpenAPIError).Body(), err)
	}
	assert.Equal(200, httpresp.StatusCode)
	...
	if err != nil {
		t.Errorf("Cannot delete Monitor %v: Response %s: %v", monitor.GetId(), err.(common.GenericOpenAPIError).Body(), err)
	}
	assert.Equal(200, httpresp.StatusCode)
	...
	if err != nil {
		t.Errorf("Error deleting Monitor %v: Response %s: %v", monitor.GetId(), err.(common.GenericOpenAPIError).Body(), err)
	}
	assert.Equal(200, httpresp.StatusCode)
...
func TestMonitorPagination(t *testing.T) {
	...
	monitors, httpresp, err := api.ListMonitors(ctx, *datadog.NewListMonitorsOptionalParameters().
		WithPage(0).
		WithPageSize(1))
	if err != nil {
		t.Errorf("Error fetching monitors: Response %s: %v", err.(common.GenericOpenAPIError).Body(), err)
	...
	monitors, httpresp, err = api.ListMonitors(ctx, *datadog.NewListMonitorsOptionalParameters().WithIdOffset(monitor.GetId() - 1).WithPageSize(1))
	if err != nil {
		t.Errorf("Error fetching monitors: Response %s: %v", err.(common.GenericOpenAPIError).Body(), err)
	}
	assert.Equal(200, httpresp.StatusCode)
...
func TestMonitorUpdate401Error(t *testing.T) {
	...
	_, httpresp, err := api.UpdateMonitor(ctx, 121, datadog.MonitorUpdateRequest{})
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			if tc.ExpectedStatusCode == 409 {
				apiError, ok := err.(common.GenericOpenAPIError).Model().(datadog.CheckCanDeleteMonitorResponse)
				assert.True(ok)
				assert.NotEmpty(apiError.GetErrors())
			} else {
				apiError, ok := err.(common.GenericOpenAPIError).Model().(datadog.APIErrorResponse)
