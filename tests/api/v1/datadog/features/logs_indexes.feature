...
Feature: Logs Indexes
  ...
  Scenario: Create an index returns "Invalid Parameter Error" response
    ...
    And body {"daily_limit": 300000000, "exclusion_filters": [{"filter": {"query": "*", "sample_rate": 1}, "is_enabled": null, "name": "payment"}], "filter": {"query": "source:python"}, "is_rate_limited": false, "name": "main", "num_retention_days": 15}
  ...
  Scenario: Update an index returns "Invalid Parameter Error" response
    ...
    And request contains "name" parameter from "<PATH>"
    And body {"daily_limit": null, "disable_daily_limit": null, "exclusion_filters": [{"filter": {"query": "*", "sample_rate": 1}, "is_enabled": null, "name": "payment"}], "filter": {"query": "source:python"}, "num_retention_days": null}
    When the request is sent
    Then the response status is 400 Invalid Parameter Error
  ...
  Scenario: Update indexes order returns "Bad Request" response
    ...
    And body {"index_names": ["main", "payments", "web"]}
    When the request is sent
