...
func TestTags(t *testing.T) {
	...
	sentHostTags, httpresp, err := api.CreateHostTags(ctx, hostname, hostTags, *datadog.NewCreateHostTagsParameters().
		WithSource("datadog"))
	if err != nil {
	...
	getHostTags, httpresp, err := api.GetHostTags(ctx, hostname, *datadog.NewGetHostTagsParameters().
		WithSource("datadog"))
	if err != nil {
	...
	err = tests.Retry(10*time.Second, 10, func() bool {
		hostTagsList, httpresp, err := api.ListHostTags(ctx, *datadog.NewListHostTagsParameters().
			WithSource("datadog"))
		if err != nil {
	...
	updatedHostTags, httpresp, err := api.UpdateHostTags(ctx, hostname, hostTags, *datadog.NewUpdateHostTagsParameters().
		WithSource("datadog"))
	if err != nil {
	...
	hostTags.Tags = &[]string{"foo:bar", "toto:tata"}
	httpresp, err = api.DeleteHostTags(ctx, hostname, *datadog.NewDeleteHostTagsParameters().WithSource("datadog"))
	if err != nil {
...
func TestTagsListErrors(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			_, httpresp, err := Client(ctx).TagsApi.ListHostTags(ctx, *datadog.NewListHostTagsParameters().
				WithSource("nosource"))
