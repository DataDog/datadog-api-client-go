...
func TestServicesLifecycle(t *testing.T) {
	...
	testServiceData := datadog.Service{
		Attributes: &datadog.ServiceAttributes{
			Name: datadog.PtrString(fmt.Sprintf("TestServicesLifecycle-%s", RandomString(16))),
		},
		Type: datadog.PtrString("services"),
	}
	testServiceAttributes := testServiceData.GetAttributes()
	// Create Service
	serviceRsp, httpresp, err := client.ServicesApi.CreateService(ctx).Body(*datadog.NewServicePayload(testServiceData)).Execute()
	if err != nil {
	...
	serviceUpdatedRsp, httpresp, err := client.ServicesApi.PatchService(ctx, service.GetId()).Body(*datadog.NewServicePayload(service)).Execute()
	if err != nil {
...
func createTestService(t *testing.T, client *datadog.APIClient, ctx context.Context) datadog.Service {
	...
	testServiceData.Attributes.SetName(fmt.Sprintf("createTestService-%s", RandomString(16)))
...
func deleteTestService(t *testing.T, client *datadog.APIClient, ctx context.Context, id string) {
