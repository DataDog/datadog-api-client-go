...
func generateUniqueAWSLambdaAccounts(ctx context.Context, t *testing.T) (datadogV1.AWSAccount, datadogV1.AWSAccountAndLambdaRequest, datadogV1.AWSLogsServicesRequest) {
	accountID := fmt.Sprintf("66%d", tests.ClockFromContext(ctx).Now().Unix())
	var uniqueAWSAccount = datadogV1.AWSAccount{
		AccountId:                     &accountID,
...
func TestAddAndSaveAWSLogs(t *testing.T) {
	...
	api := datadogV1.NewAWSLogsIntegrationApi(Client(ctx))
	...
	_, httpresp, err := api.CreateAWSLambdaARN(ctx, testLambdaAcc)
	if err != nil {
		t.Fatalf("Error adding lamda ARN: Response %s: %v", err.(datadog.GenericOpenAPIError).Body(), err)
	}
	assert.Equal(200, httpresp.StatusCode)

	_, httpresp, err = api.EnableAWSLogServices(ctx, testServices)
	if err != nil {
...
func TestListAndDeleteAWSLogs(t *testing.T) {
	...
	addOutput, httpresp, err := api.CreateAWSLambdaARN(ctx, testLambdaAcc)
	if err != nil {
	...
	listOutput1, _, err := api.ListAWSLogsIntegrations(ctx)
	if err != nil {
		t.Fatalf("Error listing log services: Response %s: %v", err.(datadog.GenericOpenAPIError).Body(), err)
	}
	assert.Equal(200, httpresp.StatusCode)
	...
	deleteOutput, httpresp, err := api.DeleteAWSLambdaARN(ctx, testLambdaAcc)
	if err != nil {
	...
	listOutput2, httpresp, err := api.ListAWSLogsIntegrations(ctx)
	if err != nil {
		t.Fatalf("Error listing log services: Response %s: %v", err.(datadog.GenericOpenAPIError).Body(), err)
	}
	assert.Equal(200, httpresp.StatusCode)

	var listOfARNs2 []datadogV1.AWSLogsLambda
...
func TestCheckServicesAsync(t *testing.T) {
	...
	status, httpresp, err := api.CheckAWSLogsServicesAsync(ctx, testServices)
	if err != nil {
...
func TestAWSLogsList400Error(t *testing.T) {
	...
	// 400 Bad Request
	_, httpresp, err := api.ListAWSLogsIntegrations(ctx)
...
func TestAWSLogsAdd400Error(t *testing.T) {
	...
	// 400 Bad Request
	_, httpresp, err := api.CreateAWSLambdaARN(ctx, datadogV1.AWSAccountAndLambdaRequest{})
	assert.Equal(400, httpresp.StatusCode)
	apiError, ok := err.(datadog.GenericOpenAPIError).Model().(datadogV1.APIErrorResponse)
	assert.True(ok)
	assert.NotEmpty(apiError.GetErrors())
...
func TestAWSLogsAdd403Error(t *testing.T) {
	...
	// 403 Forbidden
	_, httpresp, err := api.CreateAWSLambdaARN(context.Background(), datadogV1.AWSAccountAndLambdaRequest{})
	assert.Equal(403, httpresp.StatusCode)
	apiError, ok := err.(datadog.GenericOpenAPIError).Model().(datadogV1.APIErrorResponse)
	assert.True(ok)
	assert.NotEmpty(apiError.GetErrors())
...
func TestAWSLogsDelete400Error(t *testing.T) {
	...
	// 400 Bad Request
	_, httpresp, err := api.DeleteAWSLambdaARN(ctx, datadogV1.AWSAccountAndLambdaRequest{})
	assert.Equal(400, httpresp.StatusCode)
	apiError, ok := err.(datadog.GenericOpenAPIError).Model().(datadogV1.APIErrorResponse)
	assert.True(ok)
	assert.NotEmpty(apiError.GetErrors())
...
func TestAWSLogsDelete403Error(t *testing.T) {
	...
	// 403 Forbidden
	_, httpresp, err := api.DeleteAWSLambdaARN(context.Background(), datadogV1.AWSAccountAndLambdaRequest{})
	assert.Equal(403, httpresp.StatusCode)
	apiError, ok := err.(datadog.GenericOpenAPIError).Model().(datadogV1.APIErrorResponse)
	assert.True(ok)
	assert.NotEmpty(apiError.GetErrors())
...
func TestAWSLogsServicesEnableErrors(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"400 Bad Request": {WithTestAuth, datadogV1.AWSLogsServicesRequest{}, 400},
		"403 Forbidden":   {WithFakeAuth, datadogV1.AWSLogsServicesRequest{}, 403},
	}
...
func TestAWSLogsLambdaCheckErrors(t *testing.T) {
	...
	//	ExpectedStatusCode int
	//}{
	//	"400 Bad Request": {WithTestAuth, datadogV1.AWSAccountAndLambdaRequest{}, 400},
	//	"403 Forbidden":   {WithFakeAuth, datadogV1.AWSAccountAndLambdaRequest{}, 403},
	//}

	//for name, tc := range testCases {
	//	t.Run(name, func(t *testing.T) {
	//		_, httpresp, err := api.CheckAWSLogsLambdaAsync(ctx, tc.Body)
	//		assert.Equal(tc.ExpectedStatusCode, httpresp.StatusCode)
	//		apiError, ok := err.(datadog.GenericOpenAPIError).Model().(datadogV1.APIErrorResponse)
	//		assert.True(ok)
	//		assert.NotEmpty(apiError.GetErrors())
