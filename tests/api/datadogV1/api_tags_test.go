...
func TestTags(t *testing.T) {
	...
	api := datadogV1.NewTagsApi(Client(ctx))
	...
	// test methods
	hostTags := datadogV1.HostTags{
		Tags: []string{"test:client_go"},
	}
	sentHostTags, httpresp, err := api.CreateHostTags(ctx, hostname, hostTags, *datadogV1.NewCreateHostTagsOptionalParameters().
		WithSource("datadog"))
	if err != nil {
	...
	getHostTags, httpresp, err := api.GetHostTags(ctx, hostname, *datadogV1.NewGetHostTagsOptionalParameters().
		WithSource("datadog"))
	if err != nil {
	...
	err = tests.Retry(10*time.Second, 10, func() bool {
		hostTagsList, httpresp, err := api.ListHostTags(ctx, *datadogV1.NewListHostTagsOptionalParameters().
			WithSource("datadog"))
		if err != nil {
	...
	updatedHostTags, httpresp, err := api.UpdateHostTags(ctx, hostname, hostTags, *datadogV1.NewUpdateHostTagsOptionalParameters().
		WithSource("datadog"))
	if err != nil {
	...
	hostTags.Tags = []string{"foo:bar", "toto:tata"}
	httpresp, err = api.DeleteHostTags(ctx, hostname, *datadogV1.NewDeleteHostTagsOptionalParameters().WithSource("datadog"))
	if err != nil {
...
func TestTagsListErrors(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			_, httpresp, err := api.ListHostTags(ctx, *datadogV1.NewListHostTagsOptionalParameters().
				WithSource("nosource"))
...
func TestTagsCreateErrors(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			_, httpresp, err := api.CreateHostTags(ctx, "notahostname1234", datadogV1.HostTags{})
			assert.Equal(tc.ExpectedStatusCode, httpresp.StatusCode)
			apiError, ok := err.(datadog.GenericOpenAPIError).Model().(datadogV1.APIErrorResponse)
			assert.True(ok)
			assert.NotEmpty(apiError.GetErrors())
...
func TestTagsUpdateErrors(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			_, httpresp, err := api.UpdateHostTags(ctx, "notahostname1234", datadogV1.HostTags{})
			assert.Equal(tc.ExpectedStatusCode, httpresp.StatusCode)
			apiError, ok := err.(datadog.GenericOpenAPIError).Model().(datadogV1.APIErrorResponse)
			assert.True(ok)
			assert.NotEmpty(apiError.GetErrors())
