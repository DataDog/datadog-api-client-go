...
func TestListOrgs(t *testing.T) {
	...
	api := datadogV1.NewOrganizationsApi(Client(ctx))
	defer gock.Off()
...
func TestCreateOrg(t *testing.T) {
	...
	// Get mocked request data
	createBody := datadogV1.OrganizationCreateBody{
	...
	getOrgResp, _, err := api.CreateChildOrg(ctx, createBody)
	if err != nil {
...
func TestUploadOrgIdpMeta(t *testing.T) {
	...
	idpResp, _, err := api.UploadIdPForOrg(ctx, orgPubID, file)
	if err != nil {
...
func TestOrgsCreateErrors(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"400 Bad Request": {WithTestAuth, datadogV1.OrganizationCreateBody{}, 400},
		"403 Forbidden":   {WithFakeAuth, datadogV1.OrganizationCreateBody{}, 403},
	}
...
func TestOrgsUpdateErrors(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			_, httpresp, err := api.UpdateOrg(ctx, "lsqdkjf", datadogV1.Organization{})
			assert.Equal(tc.ExpectedStatusCode, httpresp.StatusCode)
			apiError, ok := err.(datadog.GenericOpenAPIError).Model().(datadogV1.APIErrorResponse)
			assert.True(ok)
			assert.NotEmpty(apiError.GetErrors())
...
func TestOrgsUploadIdpErrors(t *testing.T) {
	...
	for name, tc := range testCases {
		...
		t.Run(name, func(t *testing.T) {
			...
			_, httpresp, err := api.UploadIdPForOrg(ctx, *tests.UniqueEntityName(ctx, t), file)
...
func TestOrgsUploadIdp415Error(t *testing.T) {
	...
	_, httpresp, err := api.UploadIdPForOrg(ctx, "id", file)
