...
Feature: Synthetics
  ...
  @replay-only @skip-validation @team:DataDog/synthetics-orchestrating-managing
  ...
  @team:DataDog/synthetics-orchestrating-managing
  Scenario: Create an API test with MCP steps returns "OK - Returns the created test details." response
    Given new "CreateSyntheticsAPITest" request
    And body from file "synthetics_api_test_mcp_payload.json"
    When the request is sent
    Then the response status is 200 OK - Returns the created test details.
    And the response "name" is equal to "{{ unique }}"
    And the response "config.steps[0].subtype" is equal to "mcp"
    And the response "config.steps[0].request.callType" is equal to "init"
    And the response "config.steps[0].request.mcpProtocolVersion" is equal to "2025-06-18"
    And the response "config.steps[1].subtype" is equal to "mcp"
    And the response "config.steps[1].request.callType" is equal to "tool_list"
    And the response "config.steps[2].subtype" is equal to "mcp"
    And the response "config.steps[2].request.callType" is equal to "tool_call"
    And the response "config.steps[2].request.toolName" is equal to "search"
  ...
  @skip @team:DataDog/synthetics-orchestrating-managing
  ...
  @replay-only @skip-validation @team:DataDog/synthetics-orchestrating-managing @with-pagination
