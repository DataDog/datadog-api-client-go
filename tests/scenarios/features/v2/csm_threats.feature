...
Feature: CSM Threats
  ...
  Scenario: Batch update CSM Threats Agent policies returns "Bad Request" response
    Given there is a valid "policy_rc" in the system
    And new "BatchUpdateCSMThreatsAgentPolicy" request
    And body with value {"data": {"attributes": {"policies": [{"description": "My agent policy", "enabled": true, "hostTags": [], "id": "{{ policy.data.id }}", "name": ""}]}, "id": "batch_update_req", "type": "policies"}}
    When the request is sent
    Then the response status is 400 Bad Request

  @skip @team:DataDog/k9-cloud-security-platform @team:DataDog/k9-cws-backend
  Scenario: Batch update CSM Threats Agent policies returns "Concurrent Modification" response
    Given new "BatchUpdateCSMThreatsAgentPolicy" request
    And body with value {"data": {"attributes": {"policies": [{"delete": true, "description": "My agent policy", "enabled": true, "hostTags": [], "id": "mdh-tah-lj9", "name": "my_agent_policy", "priority": 10}]}, "id": "batch_update_req", "type": "policies"}}
    When the request is sent
    Then the response status is 409 Concurrent Modification

  @team:DataDog/k9-cloud-security-platform @team:DataDog/k9-cws-backend
  Scenario: Batch update CSM Threats Agent policies returns "Not Found" response
    Given new "BatchUpdateCSMThreatsAgentPolicy" request
    And body with value {"data": {"attributes": {"policies": [{"delete": true, "description": "My agent policy", "enabled": true, "hostTags": [], "id": "mdh-tah-lj9", "name": "my_agent_policy", "priority": 10}]}, "id": "batch_update_req", "type": "policies"}}
    When the request is sent
  ...
  @team:DataDog/k9-cloud-security-platform @team:DataDog/k9-cws-backend
  Scenario: Batch update CSM Threats Agent policies returns "OK" response
    Given there is a valid "policy_rc" in the system
    And new "BatchUpdateCSMThreatsAgentPolicy" request
    And body with value {"data": {"attributes": {"policies": [{"delete": false, "description": "Updated agent policy", "enabled": true, "hostTags": ["env:test"], "id": "{{ policy.data.id }}", "name": "updated_agent_policy", "priority": 20}]}, "id": "batch_update_req", "type": "policies"}}
    When the request is sent
  ...
  Scenario: Create a CSM Threats Agent policy returns "Bad Request" response
    ...
    And body with value {"data": {"attributes": {"description": "My agent policy", "enabled": true, "hostTags": [], "name": ""}, "type": "policy"}}
    When the request is sent
    Then the response status is 400 Bad Request
  ...
  Scenario: Create a CSM Threats Agent policy returns "Conflict" response
    Given new "CreateCSMThreatsAgentPolicy" request
    And body with value {"data": {"attributes": {"description": "My agent policy", "enabled": true, "hostTags": [], "name": "my_agent_policy", "priority": 10}, "type": "policy"}}
    When the request is sent
  ...
  Scenario: Update a CSM Threats Agent policy returns "Bad Request" response
    ...
    And request contains "policy_id" parameter from "policy.data.id"
    And body with value {"data": {"attributes": {"description": "My agent policy", "enabled": true, "hostTags": [], "name": ""}, "id": "{{ policy.data.id }}", "type": "policy"}}
    When the request is sent
    Then the response status is 400 Bad Request
  ...
  Scenario: Update a CSM Threats Agent policy returns "Concurrent Modification" response
    ...
    And new "UpdateCSMThreatsAgentPolicy" request
    And request contains "policy_id" parameter from "policy.data.id"
    And body with value {"data": {"attributes": {"description": "My agent policy", "enabled": true, "hostTags": [], "name": "my_agent_policy", "priority": 10}, "id": "{{ policy.data.id }}", "type": "policy"}}
    When the request is sent
    Then the response status is 409 Concurrent Modification
  ...
  Scenario: Update a CSM Threats Agent policy returns "Not Found" response
    ...
    And request contains "policy_id" parameter with value "non-existent-policy-id"
    And body with value {"data": {"attributes": {"description": "My agent policy", "enabled": true, "hostTags": [], "name": "my_agent_policy", "priority": 10}, "id": "non-existent-policy-id", "type": "policy"}}
    When the request is sent
    Then the response status is 404 Not Found
  ...
  Scenario: Update a CSM Threats Agent policy returns "OK" response
    ...
    And new "UpdateCSMThreatsAgentPolicy" request
    And request contains "policy_id" parameter from "policy.data.id"
    And body with value {"data": {"attributes": {"description": "Updated agent policy", "enabled": true, "hostTags": ["env:test"], "name": "updated_agent_policy", "priority": 20}, "id": "{{ policy.data.id }}", "type": "policy"}}
    When the request is sent
    Then the response status is 200 OK
  ...
  Scenario: Update a CSM Threats Agent rule returns "Bad Request" response
    ...
    And request contains "agent_rule_id" parameter from "agent_rule.data.id"
    And body with value {"data": {"attributes": {"description": "My Agent rule", "enabled": true, "expression": "exec.file.name", "policy_id": "{{ policy.data.id }}", "product_tags": []}, "id": "{{ agent_rule.data.id }}", "type": "agent_rule"}}
    When the request is sent
    Then the response status is 400 Bad Request
  ...
  Scenario: Update a Cloud Workload Security Agent rule returns "OK" response
    ...
    And new "UpdateCloudWorkloadSecurityAgentRule" request
    And request contains "agent_rule_id" parameter from "agent_rule.data.id"
    And body with value {"data": {"attributes": {"description": "Updated Agent rule", "enabled": true, "expression": "exec.file.name == \"sh\"", "policy_id": "{{ policy.data.id }}", "product_tags": []}, "id": "{{ agent_rule.data.id }}", "type": "agent_rule"}}
    When the request is sent
    Then the response status is 200 OK
