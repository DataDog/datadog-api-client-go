...
Feature: Application Security
  ...
  Scenario: Update a WAF Custom Rule returns "Bad Request" response
    ...
    And body with value {"data": {"type": "custom_rule", "attributes": {"blocking": false, "conditions": [{"operator": "match_regex", "parameters": { "inputs": [ { "address": "server.request.query", "key_path": [ "id" ] } ], "regex": "\\" } } ], "enabled": false, "name": "test", "path_glob": "/test", "scope": [ { "env": "test", "service": "test" } ], "tags": { "category": "attack_attempt", "test": "1", "type": "test"}}}}
    When the request is sent
    Then the response status is 400 Bad Request
  ...
  Scenario: Update a WAF Custom Rule returns "OK" response
    ...
    And new "UpdateApplicationSecurityWafCustomRule" request
    And request contains "custom_rule_id" parameter from "custom_rule.data.id"
    And body with value {"data": {"type": "custom_rule", "attributes": {"blocking": false, "conditions": [{"operator": "match_regex", "parameters": { "inputs": [ { "address": "server.request.query", "key_path": [ "id" ] } ], "regex": "badactor" } } ], "enabled": false, "name": "test", "path_glob": "/test", "scope": [ { "env": "test", "service": "test" } ], "tags": { "category": "attack_attempt", "test": "1", "type": "test"}}}}
    When the request is sent
    Then the response status is 200 OK
