...
Feature: Rum Metrics
  ...
  Scenario: Create a rum-based metric returns "Bad Request" response
    ...
    And body with value {"data": {"id": "rum.actions.invalid", "type": "rum_metrics", "attributes": {"event_type": "action"}}}
  ...
  Scenario: Update a rum-based metric returns "Bad Request" response
    ...
    And request contains "metric_id" parameter from "rum_metric.data.id"
    And body with value {"data": {"id": "rum.sessions.webui.count", "type": "rum_metrics", "attributes": {"compute": {"include_percentiles": "NaN"}}}}
    When the request is sent
    Then the response status is 400 Bad Request
