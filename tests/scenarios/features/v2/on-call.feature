...
Feature: On-Call
  ...
  Scenario: Delete on-call schedule returns "No Content" response
    ...
    And request contains "schedule_uuid" parameter from "schedule.data.id"
    When the request is sent
  ...
  Scenario: Update on-call schedule returns "OK" response
    ...
    And there is a valid "user" in the system
    And request contains "schedule_uuid" parameter from "schedule.data.id"
    And body with value {"data": { "id": "{{ schedule.data.id }}", "attributes": {"layers": [{"effective_date": "2025-02-03T05:00:00Z", "end_date": "2025-12-31T00:00:00Z", "interval": {"seconds": 300}, "members": [{"user": {"id": "{{user.data.id}}"}}], "name": "Layer 1", "restrictions": [{"end_day": "friday", "end_time": "17:00:00", "start_day": "monday", "start_time": "09:00:00"}], "rotation_start": "2025-02-01T00:00:00Z"}], "name": "{{ unique }}", "tags": ["tag1", "tag2", "tag3"], "time_zone": "America/New_York"}, "relationships": {"teams": {"data": [{"id": "{{team.data.id}}", "type": "teams"}]}}, "type": "schedules"}}
    When the request is sent
    Then the response status is 200 OK
