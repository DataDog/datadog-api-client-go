...
func TestRetryDelayDoesNotWinAfterDeadline(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		delay <- time.Now()
		if retryDelayElapsed(ctx, delay) {
			t.Fatal("retry delay won after deadline")
		}
	}
}

func TestRetryJitterSaturatesAtMaxDuration(t *testing.T) {
	retryDuration := time.Duration(math.MaxInt64) - time.Second
	if got := addRetryJitter(retryDuration, time.Second+1); got != time.Duration(math.MaxInt64) {
		t.Fatalf("duration = %s; want %s", got, time.Duration(math.MaxInt64))
	}
}
