...
// WriteDatastoreItem Write datastore item.
// Creates or replaces an item in a datastore by its key.
func (a *ActionsDatastoresApi) WriteDatastoreItem(ctx _context.Context, datastoreId string, body PutAppsDatastoreItemRequest) (PutAppsDatastoreItemResponse, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod  = _nethttp.MethodPost
		localVarPostBody    interface{}
	...
	localBasePath, err := a.Client.Cfg.ServerURLWithContext(ctx, "v2.ActionsDatastoresApi.WriteDatastoreItem")
	if err != nil {
		return localVarReturnValue, nil, datadog.GenericOpenAPIError{ErrorMessage: err.Error()}
	}
