...
// ObservabilityPipelineThrottleProcessor The `throttle` processor limits the rate of events using a time-based window.
...
func (o *ObservabilityPipelineThrottleProcessor) UnmarshalJSON(bytes []byte) (err error) {
	...
	if err = datadog.Unmarshal(bytes, &additionalProperties); err == nil {
		datadog.DeleteKeys(additionalProperties, &[]string{"id", "include", "inputs", "threshold", "type", "window"})
	} else {
		return err
	}
