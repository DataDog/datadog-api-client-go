...
// ValidateDashboardWidgets Validate dashboard widgets.
// Validate dashboard widgets without creating or updating a dashboard.
func (a *DashboardsApi) ValidateDashboardWidgets(ctx _context.Context, body DashboardWidgetValidationRequest) (DashboardWidgetValidationResponse, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod  = _nethttp.MethodPost
		localVarPostBody    interface{}
		localVarReturnValue DashboardWidgetValidationResponse
	)

	operationId := "v2.ValidateDashboardWidgets"
	isOperationEnabled := a.Client.Cfg.IsUnstableOperationEnabled(operationId)
	if !isOperationEnabled {
		return localVarReturnValue, nil, datadog.GenericOpenAPIError{ErrorMessage: _fmt.Sprintf("Unstable operation '%s' is disabled", operationId)}
	}
	...
	localBasePath, err := a.Client.Cfg.ServerURLWithContext(ctx, "v2.DashboardsApi.ValidateDashboardWidgets")
	if err != nil {
		return localVarReturnValue, nil, datadog.GenericOpenAPIError{ErrorMessage: err.Error()}
	}

	localVarPath := localBasePath + "/api/v2/dashboard/widgets/validate"
