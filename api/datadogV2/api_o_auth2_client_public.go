...
// GetOIDCDiscoveryDocument Get OpenID Connect provider metadata.
// Retrieve OpenID Connect provider metadata for the OAuth2 v2 token endpoint.
func (a *OAuth2ClientPublicApi) GetOIDCDiscoveryDocument(ctx _context.Context) (OIDCDiscoveryDocument, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod  = _nethttp.MethodGet
		localVarPostBody    interface{}
		localVarReturnValue OIDCDiscoveryDocument
	)

	operationId := "v2.GetOIDCDiscoveryDocument"
	isOperationEnabled := a.Client.Cfg.IsUnstableOperationEnabled(operationId)
	if !isOperationEnabled {
		return localVarReturnValue, nil, datadog.GenericOpenAPIError{ErrorMessage: _fmt.Sprintf("Unstable operation '%s' is disabled", operationId)}
	}
	...
	localBasePath, err := a.Client.Cfg.ServerURLWithContext(ctx, "v2.OAuth2ClientPublicApi.GetOIDCDiscoveryDocument")
	if err != nil {
		return localVarReturnValue, nil, datadog.GenericOpenAPIError{ErrorMessage: err.Error()}
	}

	localVarPath := localBasePath + "/api/v2/oauth2/.well-known/openid-configuration"
