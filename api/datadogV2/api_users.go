...
// GetUserIdentityProviders Get identity provider overrides for a user.
...
func (a *UsersApi) GetUserIdentityProviders(ctx _context.Context, userId string) (UserOverrideIdentityProvidersResponse, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod  = _nethttp.MethodGet
		localVarPostBody    interface{}
		localVarReturnValue UserOverrideIdentityProvidersResponse
	)

	localBasePath, err := a.Client.Cfg.ServerURLWithContext(ctx, "v2.UsersApi.GetUserIdentityProviders")
	if err != nil {
		return localVarReturnValue, nil, datadog.GenericOpenAPIError{ErrorMessage: err.Error()}
	}

	localVarPath := localBasePath + "/api/v2/users/{user_id}/identity_providers"
	localVarPath = datadog.ReplacePathParameter(localVarPath, "{user_id}", _neturl.PathEscape(datadog.ParameterToString(userId, "")))
...
// UpdateUserIdentityProviders Update identity provider overrides for a user.
...
func (a *UsersApi) UpdateUserIdentityProviders(ctx _context.Context, userId string, body UpdateUserIdentityProvidersRequest) (*_nethttp.Response, error) {
	var (
		localVarHTTPMethod = _nethttp.MethodPatch
		localVarPostBody   interface{}
	)

	localBasePath, err := a.Client.Cfg.ServerURLWithContext(ctx, "v2.UsersApi.UpdateUserIdentityProviders")
	if err != nil {
		return nil, datadog.GenericOpenAPIError{ErrorMessage: err.Error()}
	}

	localVarPath := localBasePath + "/api/v2/users/{user_id}/relationships/identity_providers"
	localVarPath = datadog.ReplacePathParameter(localVarPath, "{user_id}", _neturl.PathEscape(datadog.ParameterToString(userId, "")))
