...
// EventPayloadAttributes - JSON object for custom attributes. Schema is different per event category.
...
func (obj *EventPayloadAttributes) UnmarshalJSON(data []byte) error {
	...
	if err == nil {
		if obj.AlertEventCustomAttributes != nil && obj.AlertEventCustomAttributes.UnparsedObject == nil {
			...
			if string(jsonAlertEventCustomAttributes) == "{}" && string(data) != "{}" { // empty struct
