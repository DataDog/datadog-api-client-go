...
// DatabricksIntegrationAccountPatAuthUpdate Databricks personal access token authentication. Deprecated: accepted only on accounts that already use it, and never on creation. Use `databricks-oauth` or `private-action-runner` instead. An update replaces this object entirely, so `token` must be sent with it.
//
