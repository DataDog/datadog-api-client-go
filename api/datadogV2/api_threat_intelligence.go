...
// AddSTIXThreatIntelOptionalParameters holds optional parameters for AddSTIXThreatIntel.
type AddSTIXThreatIntelOptionalParameters struct {
	ContentEncoding *STIXContentEncoding
}

// NewAddSTIXThreatIntelOptionalParameters creates an empty struct for parameters.
func NewAddSTIXThreatIntelOptionalParameters() *AddSTIXThreatIntelOptionalParameters {
	this := AddSTIXThreatIntelOptionalParameters{}
	return &this
}

// WithContentEncoding sets the corresponding parameter name and returns the struct.
func (r *AddSTIXThreatIntelOptionalParameters) WithContentEncoding(contentEncoding STIXContentEncoding) *AddSTIXThreatIntelOptionalParameters {
	r.ContentEncoding = &contentEncoding
	return r
}

// AddSTIXThreatIntel Ingest STIX threat intelligence.
...
func (a *ThreatIntelligenceApi) AddSTIXThreatIntel(ctx _context.Context, tiVendor string, body STIXBundleRequest, o ...AddSTIXThreatIntelOptionalParameters) (STIXIngestResponse, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod  = _nethttp.MethodPost
		localVarPostBody    interface{}
		localVarReturnValue STIXIngestResponse
		optionalParams      AddSTIXThreatIntelOptionalParameters
	)

	if len(o) > 1 {
		return localVarReturnValue, nil, datadog.ReportError("only one argument of type AddSTIXThreatIntelOptionalParameters is allowed")
	}
	if len(o) == 1 {
		optionalParams = o[0]
	}

	operationId := "v2.AddSTIXThreatIntel"
	...
	localBasePath, err := a.Client.Cfg.ServerURLWithContext(ctx, "v2.ThreatIntelligenceApi.AddSTIXThreatIntel")
	if err != nil {
	...
	if localVarHTTPResponse.StatusCode >= 300 {
		...
		if localVarHTTPResponse.StatusCode == 400 || localVarHTTPResponse.StatusCode == 413 || localVarHTTPResponse.StatusCode == 429 || localVarHTTPResponse.StatusCode == 502 || localVarHTTPResponse.StatusCode == 503 {
