...
// SubmitStixThreatIntelOptionalParameters holds optional parameters for SubmitStixThreatIntel.
type SubmitStixThreatIntelOptionalParameters struct {
	ContentEncoding *STIXContentEncoding
}

// NewSubmitStixThreatIntelOptionalParameters creates an empty struct for parameters.
func NewSubmitStixThreatIntelOptionalParameters() *SubmitStixThreatIntelOptionalParameters {
	this := SubmitStixThreatIntelOptionalParameters{}
	return &this
}

// WithContentEncoding sets the corresponding parameter name and returns the struct.
func (r *SubmitStixThreatIntelOptionalParameters) WithContentEncoding(contentEncoding STIXContentEncoding) *SubmitStixThreatIntelOptionalParameters {
	r.ContentEncoding = &contentEncoding
	return r
}

// SubmitStixThreatIntel Ingest STIX threat intelligence.
...
func (a *ThreatIntelligenceApi) SubmitStixThreatIntel(ctx _context.Context, tiVendor string, body STIXBundleRequest, o ...SubmitStixThreatIntelOptionalParameters) (STIXIngestResponse, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod  = _nethttp.MethodPost
		localVarPostBody    interface{}
		localVarReturnValue STIXIngestResponse
		optionalParams      SubmitStixThreatIntelOptionalParameters
	)

	if len(o) > 1 {
		return localVarReturnValue, nil, datadog.ReportError("only one argument of type SubmitStixThreatIntelOptionalParameters is allowed")
	}
	if len(o) == 1 {
		optionalParams = o[0]
	}

	operationId := "v2.SubmitStixThreatIntel"
	...
	localBasePath, err := a.Client.Cfg.ServerURLWithContext(ctx, "v2.ThreatIntelligenceApi.SubmitStixThreatIntel")
	if err != nil {
	...
	if localVarHTTPResponse.StatusCode >= 300 {
		...
		if localVarHTTPResponse.StatusCode == 413 || localVarHTTPResponse.StatusCode == 502 || localVarHTTPResponse.StatusCode == 503 {
