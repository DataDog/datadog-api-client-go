...
func main() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ElasticCloudCloudCostManagementApi.UpdateElasticCloudCcmAccount`: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}

	responseContent, _ := json.MarshalIndent(resp, "", "  ")
	fmt.Fprintf(os.Stdout, "Response from `ElasticCloudCloudCostManagementApi.UpdateElasticCloudCcmAccount`:\n%s\n", responseContent)
}
