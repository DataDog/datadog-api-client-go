...
func main() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `AgentObservabilityApi.SearchLLMObsExperimentation`: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}

	responseContent, _ := json.MarshalIndent(resp, "", "  ")
	fmt.Fprintf(os.Stdout, "Response from `AgentObservabilityApi.SearchLLMObsExperimentation`:\n%s\n", responseContent)
}
