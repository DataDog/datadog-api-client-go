...
func main() {
	body := datadogV2.LLMObsAnnotationQueueRequest{
		Data: datadogV2.LLMObsAnnotationQueueDataRequest{
			Attributes: datadogV2.LLMObsAnnotationQueueDataAttributesRequest{
				...
				RestrictToAssignees: datadog.PtrBool(false),
				RestrictToReviewers: datadog.PtrBool(true),
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `AgentObservabilityApi.CreateLLMObsAnnotationQueue`: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}

	responseContent, _ := json.MarshalIndent(resp, "", "  ")
	fmt.Fprintf(os.Stdout, "Response from `AgentObservabilityApi.CreateLLMObsAnnotationQueue`:\n%s\n", responseContent)
}
