...
func main() {
	...
	resp, r, err := api.UpdateArbitraryCostRule(ctx, 123456, body)

	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `CloudCostManagementApi.UpdateArbitraryCostRule`: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}

	responseContent, _ := json.MarshalIndent(resp, "", "  ")
	fmt.Fprintf(os.Stdout, "Response from `CloudCostManagementApi.UpdateArbitraryCostRule`:\n%s\n", responseContent)
}
