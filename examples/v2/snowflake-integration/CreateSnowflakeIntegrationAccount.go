...
func main() {
	body := datadogV2.SnowflakeIntegrationAccountCreateRequest{
		Data: datadogV2.SnowflakeIntegrationAccountCreateData{
			Attributes: datadogV2.SnowflakeIntegrationAccountCreateAttributes{
				Authentication: datadogV2.SnowflakeIntegrationAccountAuthenticationRequest{
					SnowflakeIntegrationAccountPrivateKeyAuthRequest: &datadogV2.SnowflakeIntegrationAccountPrivateKeyAuthRequest{
...
-----END PRIVATE KEY-----`,
				...
				Name: "prod-snowflake",
				Settings: datadogV2.SnowflakeIntegrationAccountSettingsRequest{
	...
	configuration.SetUnstableOperationEnabled("v2.CreateSnowflakeIntegrationAccount", true)
	apiClient := datadog.NewAPIClient(configuration)
	api := datadogV2.NewSnowflakeIntegrationApi(apiClient)
	resp, r, err := api.CreateSnowflakeIntegrationAccount(ctx, body)

	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `SnowflakeIntegrationApi.CreateSnowflakeIntegrationAccount`: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}

	responseContent, _ := json.MarshalIndent(resp, "", "  ")
	fmt.Fprintf(os.Stdout, "Response from `SnowflakeIntegrationApi.CreateSnowflakeIntegrationAccount`:\n%s\n", responseContent)
}
