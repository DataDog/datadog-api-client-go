...
func main() {
	body := datadogV2.AutomationRuleUpdateRequest{
		Data: datadogV2.AutomationRuleUpdate{
	...
	configuration.SetUnstableOperationEnabled("v2.UpdateCaseAutomationRule", true)
	apiClient := datadog.NewAPIClient(configuration)
	api := datadogV2.NewCaseManagementApi(apiClient)
	resp, r, err := api.UpdateCaseAutomationRule(ctx, "project_id", "rule_id", body)

	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `CaseManagementApi.UpdateCaseAutomationRule`: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}

	responseContent, _ := json.MarshalIndent(resp, "", "  ")
	fmt.Fprintf(os.Stdout, "Response from `CaseManagementApi.UpdateCaseAutomationRule`:\n%s\n", responseContent)
}
