...
func main() {
	...
	r, err := api.DisableFeatureFlagEnvironment(ctx, FeatureFlagDataID, EnvironmentDataID)

	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `FeatureFlagsApi.DisableFeatureFlagEnvironment`: %v\n", err)
