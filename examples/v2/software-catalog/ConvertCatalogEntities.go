...
func main() {
	...
	configuration.SetUnstableOperationEnabled("v2.ConvertCatalogEntities", true)
	apiClient := datadog.NewAPIClient(configuration)
	api := datadogV2.NewSoftwareCatalogApi(apiClient)
	resp, r, err := api.ConvertCatalogEntities(ctx, datadogV2.ENTITYSCHEMAVERSION_V3, body)

	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `SoftwareCatalogApi.ConvertCatalogEntities`: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}

	responseContent, _ := json.MarshalIndent(resp, "", "  ")
	fmt.Fprintf(os.Stdout, "Response from `SoftwareCatalogApi.ConvertCatalogEntities`:\n%s\n", responseContent)
}
