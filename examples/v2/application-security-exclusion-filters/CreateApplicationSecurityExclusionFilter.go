// Create an ASM Exclusion filter returns "OK" response

package main

import (
	"context"
	"encoding/json"
	"fmt"
	"os"

	"github.com/DataDog/datadog-api-client-go/v2/api/datadog"
	"github.com/DataDog/datadog-api-client-go/v2/api/datadogV2"
)

func main() {
	body := datadogV2.ApplicationSecurityExclusionFilterCreateRequest{
		Data: datadogV2.ApplicationSecurityExclusionFilterCreateData{
			Type: datadogV2.APPLICATIONSECURITYEXCLUSIONFILTERTYPE_EXCLUSION_FILTER,
			Attributes: datadogV2.ApplicationSecurityExclusionFilterCreateAttributes{
				Description: "my description",
				Enabled:     true,
				PathGlob:    "*",
				RulesTarget: []datadogV2.ApplicationSecurityExclusionFilterRulesTarget{
					{},
				},
				Scope: []datadogV2.ApplicationSecurityExclusionFilterScope{
					{
						Env:     datadog.PtrString("staging"),
						Service: datadog.PtrString("container-resolver"),
					},
				},
			},
		},
	}
	ctx := datadog.NewDefaultContext(context.Background())
	configuration := datadog.NewConfiguration()
	apiClient := datadog.NewAPIClient(configuration)
	api := datadogV2.NewApplicationSecurityExclusionFiltersApi(apiClient)
	resp, r, err := api.CreateApplicationSecurityExclusionFilter(ctx, body)

	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ApplicationSecurityExclusionFiltersApi.CreateApplicationSecurityExclusionFilter`: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}

	responseContent, _ := json.MarshalIndent(resp, "", "  ")
	fmt.Fprintf(os.Stdout, "Response from `ApplicationSecurityExclusionFiltersApi.CreateApplicationSecurityExclusionFilter`:\n%s\n", responseContent)
}
