...
func main() {
	...
	configuration.SetUnstableOperationEnabled("v2.MitigateGovernanceDetections", true)
	apiClient := datadog.NewAPIClient(configuration)
	api := datadogV2.NewGovernanceConsoleApi(apiClient)
	r, err := api.MitigateGovernanceDetections(ctx, body)

	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `GovernanceConsoleApi.MitigateGovernanceDetections`: %v\n", err)
