...
func main() {
	body := datadogV2.ProductAnalyticsFormulaJourneyRequest{
		Data: datadogV2.ProductAnalyticsFormulaJourneyRequestData{
			Attributes: datadogV2.ProductAnalyticsFormulaJourneyRequestAttributes{
				From: 1756425600000,
				Query: datadogV2.ProductAnalyticsFormulaJourneyQuery{
					Compute: datadogV2.ProductAnalyticsGraphQueryCompute{
						Aggregation: "count",
	...
	configuration.SetUnstableOperationEnabled("v2.QueryProductAnalyticsJourneyTimeseries", true)
	apiClient := datadog.NewAPIClient(configuration)
	api := datadogV2.NewProductAnalyticsApi(apiClient)
	resp, r, err := api.QueryProductAnalyticsJourneyTimeseries(ctx, body)

	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ProductAnalyticsApi.QueryProductAnalyticsJourneyTimeseries`: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}

	responseContent, _ := json.MarshalIndent(resp, "", "  ")
	fmt.Fprintf(os.Stdout, "Response from `ProductAnalyticsApi.QueryProductAnalyticsJourneyTimeseries`:\n%s\n", responseContent)
}
