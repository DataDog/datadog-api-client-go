...
func main() {
	body := datadogV2.ProductAnalyticsJourneyFunnelRequest{
		Data: datadogV2.ProductAnalyticsJourneyFunnelRequestData{
			Attributes: datadogV2.ProductAnalyticsJourneyFunnelRequestAttributes{
				ExcludeAnonymousTraffic: datadog.PtrBool(false),
				From:                    1756425600000,
				Query: datadogV2.ProductAnalyticsJourneyFunnelQuery{
					Compute: &datadogV2.ProductAnalyticsJourneyFunnelCompute{},
			...
			Type: datadogV2.PRODUCTANALYTICSJOURNEYREQUESTTYPE_JOURNEY_REQUEST,
		},
	}
	ctx := datadog.NewDefaultContext(context.Background())
	configuration := datadog.NewConfiguration()
	configuration.SetUnstableOperationEnabled("v2.QueryProductAnalyticsJourneyFunnel", true)
	apiClient := datadog.NewAPIClient(configuration)
	api := datadogV2.NewProductAnalyticsApi(apiClient)
	resp, r, err := api.QueryProductAnalyticsJourneyFunnel(ctx, body)

	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ProductAnalyticsApi.QueryProductAnalyticsJourneyFunnel`: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}

	responseContent, _ := json.MarshalIndent(resp, "", "  ")
	fmt.Fprintf(os.Stdout, "Response from `ProductAnalyticsApi.QueryProductAnalyticsJourneyFunnel`:\n%s\n", responseContent)
}
