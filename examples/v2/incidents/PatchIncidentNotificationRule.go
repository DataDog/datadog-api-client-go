...
func main() {
	...
	configuration.SetUnstableOperationEnabled("v2.PatchIncidentNotificationRule", true)
	apiClient := datadog.NewAPIClient(configuration)
	api := datadogV2.NewIncidentsApi(apiClient)
	resp, r, err := api.PatchIncidentNotificationRule(ctx, uuid.MustParse("00000000-0000-0000-0000-000000000001"), body, *datadogV2.NewPatchIncidentNotificationRuleOptionalParameters())

	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `IncidentsApi.PatchIncidentNotificationRule`: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}

	responseContent, _ := json.MarshalIndent(resp, "", "  ")
	fmt.Fprintf(os.Stdout, "Response from `IncidentsApi.PatchIncidentNotificationRule`:\n%s\n", responseContent)
}
