...
func main() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ElasticCloudMonitoringApi.UpdateElasticCloudMonitoringAccount`: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}

	responseContent, _ := json.MarshalIndent(resp, "", "  ")
	fmt.Fprintf(os.Stdout, "Response from `ElasticCloudMonitoringApi.UpdateElasticCloudMonitoringAccount`:\n%s\n", responseContent)
}
