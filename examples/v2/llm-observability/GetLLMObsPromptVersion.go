...
func main() {
	...
	resp, r, err := api.GetLLMObsPromptVersion(ctx, PromptDataAttributesPromptID, PromptVersionDataAttributesVersion)

	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `LLMObservabilityApi.GetLLMObsPromptVersion`: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}

	responseContent, _ := json.MarshalIndent(resp, "", "  ")
	fmt.Fprintf(os.Stdout, "Response from `LLMObservabilityApi.GetLLMObsPromptVersion`:\n%s\n", responseContent)
}
