...
func main() {
		...
		Data: datadogV2.TwilioIntegrationAccountUpdateData{
			Attributes: datadogV2.TwilioIntegrationAccountUpdateAttributes{
				Authentication: &datadogV2.TwilioIntegrationAccountAuthenticationUpdate{
					TwilioIntegrationAccountBasicAuthUpdate: &datadogV2.TwilioIntegrationAccountBasicAuthUpdate{
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `TwilioIntegrationApi.UpdateTwilioIntegrationAccount`: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}

	responseContent, _ := json.MarshalIndent(resp, "", "  ")
	fmt.Fprintf(os.Stdout, "Response from `TwilioIntegrationApi.UpdateTwilioIntegrationAccount`:\n%s\n", responseContent)
}
