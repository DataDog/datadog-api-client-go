...
func main() {
	...
	body := datadogV2.InboxRuleUpdateRequest{
		Data: datadogV2.InboxRuleDataUpdate{
	...
	configuration.SetUnstableOperationEnabled("v2.UpdateSecurityFindingsAutomationInboxRule", true)
	apiClient := datadog.NewAPIClient(configuration)
	api := datadogV2.NewSecurityMonitoringApi(apiClient)
	resp, r, err := api.UpdateSecurityFindingsAutomationInboxRule(ctx, ValidInboxRuleDataID, body)

	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `SecurityMonitoringApi.UpdateSecurityFindingsAutomationInboxRule`: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}

	responseContent, _ := json.MarshalIndent(resp, "", "  ")
	fmt.Fprintf(os.Stdout, "Response from `SecurityMonitoringApi.UpdateSecurityFindingsAutomationInboxRule`:\n%s\n", responseContent)
}
