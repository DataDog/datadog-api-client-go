...
func main() {
	...
	configuration.SetUnstableOperationEnabled("v2.PostSCAResult", true)
	apiClient := datadog.NewAPIClient(configuration)
	api := datadogV2.NewStaticAnalysisApi(apiClient)
	r, err := api.PostSCAResult(ctx, body)

	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `StaticAnalysisApi.PostSCAResult`: %v\n", err)
