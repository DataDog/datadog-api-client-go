...
func main() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `MetricsApi.ListTagIndexingRulesForMetric`: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}

	responseContent, _ := json.MarshalIndent(resp, "", "  ")
	fmt.Fprintf(os.Stdout, "Response from `MetricsApi.ListTagIndexingRulesForMetric`:\n%s\n", responseContent)
}
