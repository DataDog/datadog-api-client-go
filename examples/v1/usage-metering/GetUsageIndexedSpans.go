...
func main() {
	...
	resp, r, err := apiClient.UsageMeteringApi.GetUsageIndexedSpans(ctx, time.Now().AddDate(0, 0, -5), *datadog.NewGetUsageIndexedSpansOptionalParameters().WithEndHr(time.Now().AddDate(0, 0, -3)))
