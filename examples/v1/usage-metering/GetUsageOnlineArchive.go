...
func main() {
	...
	resp, r, err := apiClient.UsageMeteringApi.GetUsageOnlineArchive(ctx, time.Now().AddDate(0, 0, -5), *datadog.NewGetUsageOnlineArchiveOptionalParameters().WithEndHr(time.Now().AddDate(0, 0, -3)), )
