...
func main() {
	...
	resp, r, err := apiClient.UsageMeteringApi.GetUsageSNMP(ctx, time.Now().AddDate(0, 0, -5), *datadog.NewGetUsageSNMPOptionalParameters().WithEndHr(time.Now().AddDate(0, 0, -3)))
