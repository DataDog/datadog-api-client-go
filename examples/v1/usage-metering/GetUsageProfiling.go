...
func main() {
	...
	resp, r, err := apiClient.UsageMeteringApi.GetUsageProfiling(ctx, time.Now().AddDate(0, 0, -5), *datadog.NewGetUsageProfilingOptionalParameters().WithEndHr(time.Now().AddDate(0, 0, -3)))
