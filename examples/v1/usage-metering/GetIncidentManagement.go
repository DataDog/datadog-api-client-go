...
func main() {
	...
	resp, r, err := apiClient.UsageMeteringApi.GetIncidentManagement(ctx, time.Now().AddDate(0, 0, -5), *datadog.NewGetIncidentManagementOptionalParameters().WithEndHr(time.Now().AddDate(0, 0, -3)))
