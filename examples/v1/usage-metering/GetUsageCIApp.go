...
func main() {
	...
	resp, r, err := apiClient.UsageMeteringApi.GetUsageCIApp(ctx, time.Now().AddDate(0, 0, -5), *datadog.NewGetUsageCIAppOptionalParameters().WithEndHr(time.Now().AddDate(0, 0, -3)), )
